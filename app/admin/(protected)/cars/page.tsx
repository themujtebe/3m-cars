            ...
            {cars.map((car) => (
              <div key={car.id} className="flex items-center gap-4 px-5 py-4 transition-colors hover:bg-[#f7f7f7] ">
                ...
                <div className="flex-1 min-w-0">
                  <p className="truncate text-[15px] font-semibold text-[#111] " style={{ fontFamily: "var(--font-tajawal)" }}>{car.title_ar}</p>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  <Link href={`/cars/${car.id}`} target="_blank" className="rounded-lg border border-black/[0.08] px-3 py-1.5 text-[12px] font-medium text-[#555] transition-colors hover:border-black/20 hover:text-[#111]   " style={{ fontFamily: "var(--font-tajawal)" }}>عرض</Link>
                  <Link href={`/admin/cars/${car.id}/edit`} className="flex items-center gap-1.5 rounded-lg border border-black/[0.08] px-3 py-1.5 text-[12px] font-medium text-[#555] transition-colors hover:border-black/20 hover:text-[#111]   " style={{ fontFamily: "var(--font-tajawal)" }}>
                    <Pencil className="h-3 w-3" />تعديل
                  </Link>
                  <form action={updateCarStatus}>
                    <input type="hidden" name="id" value={car.id} />
                    <input type="hidden" name="status" value={car.status === "available" ? "sold" : "available"} />
                    <button type="submit" className="flex items-center gap-1.5 rounded-lg border border-black/[0.08] px-3 py-1.5 text-[12px] font-medium text-[#555] transition-colors hover:text-[#111]   " style={{ fontFamily: "var(--font-tajawal)" }}>
                  ...
                  <form action={deleteCarAction}>
                    <input type="hidden" name="id" value={car.id} />
                    <button type="submit" className="flex items-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-[12px] font-medium text-[#a71225] transition-colors hover:bg-red-100    " style={{ fontFamily: "var(--font-tajawal)" }}>
