  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/5] overflow-hidden bg-[#f0f0f0] dark:bg-[#374151]">
        ...
        />
        {car.featured && !isSold && (
          <span className="absolute right-3 top-3 rounded-full bg-[#a71225] px-3 py-0.5 text-[10px] font-bold text-white" style={{ fontFamily: "var(--font-tajawal)" }}>
            مميزة
          </span>
        )}
        {isSold && (
      ...
      <div className="px-5 py-4">
        <h3 className="text-[18px] font-bold text-[#111] leading-snug dark:text-[#f9fafb]" style={{ fontFamily: "var(--font-tajawal)" }}>
          {car.title_ar}
        </h3>

        <div className="mt-3 flex flex-wrap gap-2">
          {[String(car.year), `${car.mileage.toLocaleString("en-US")} كم`, car.brand].map((val) => (
            <span key={val} className="rounded-md bg-[#f0f0f0] px-2.5 py-1 text-[12px] font-semibold text-[#555] dark:bg-[#374151] dark:text-[#d1d5db]" style={{ fontFamily: "var(--font-tajawal)" }}>
              {val}
            </span>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              ...
              <span className="text-[11px] font-bold text-[#aaa] dark:text-[#6b7280]">{currency}</span>
            </div>
            <span className="dm-price text-[26px] font-bold text-[#a71225]" style={{ fontFamily: "var(--font-tajawal)" }}>
